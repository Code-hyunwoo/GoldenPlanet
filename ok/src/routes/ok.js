...
function hybrid(object, isVirtual) {
  let GAData = isVirtual
    ? { ...virCommonData, ...object }
    : { ...commonData, ...object };
  isAndroid
    ? window.gascriptAndroid.GAHybrid(JSON.stringify(GAData))
    : window.webkit.messageHandlers.gascriptCallbackHandler.postMessage(
...
function sendGAPage(object) {
  try {
    ...
    if (isAndroid || isIOS) {
      ...
      })(window, document, "script", "dataLayer", "GTM-WWLQVNBR");
